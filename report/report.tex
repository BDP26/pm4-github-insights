...
Nine REST endpoints under \texttt{/api/hidden-gems/} expose the discovery engine.
...
\TODO{Report cohort results for each interval (24\,h, 168\,h, 730\,h): number
of snapshots evaluated, total repositories flagged (sig\_score\,$\geq 1.5$),
sustained count, dropped count, and precision (sustained\,/\,total). Present as
a three-row table. Values are directly readable from the cohort endpoint response.}

\subsubsection{Representative Detections}

\TODO{Present the top-5 highest-scoring repositories observed during the
evaluation period, taken directly from the live ranking dashboard (repository
scope, highest sig\_score across any window). For each entry report: full name,
programming language, sig\_score, number of stars and forks accrued in the
window, and a one-sentence description of the project. This constitutes the
primary qualitative output of the system.}
