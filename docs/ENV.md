...
| `GITHUB_TOKENS_USER` | No | _(empty)_ | Comma-separated list of GitHub PATs for user enrichment. Takes precedence over `GITHUB_TOKEN_USER`. Up to 4 tokens supported. Each token has its own 5,000 req/hour budget. |
| `GITHUB_TOKENS_REPO` | No | _(empty)_ | Comma-separated list of GitHub PATs for repo enrichment. Takes precedence over `GITHUB_TOKEN_REPO`. Up to 4 tokens supported. |
