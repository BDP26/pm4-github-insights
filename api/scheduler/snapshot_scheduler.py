...
class SnapshotScheduler:
    ...
    async def trigger(self, interval_hours: int) -> None:
        ...
        Intentionally delegates to _run_with_retry so manual triggers get the same
        retry/timeout resilience as scheduled runs.
        """
        await self._run_with_retry(interval_hours)
    ...
    async def _run_with_retry(self, interval_hours: int) -> None:
        ...
        for attempt in range(total_attempts):
            ...
            try:
                ...
                log.error(
                    "Snapshot timed out on attempt %d/%d (interval=%sh timeout=%.0fs) — not retrying",
                    attempt + 1, total_attempts, interval_hours, timeout,
                )
                raise
